	...
	<nav class="bg-white border-gray-200 px-4 lg:px-6 py-2.5 dark:bg-gray-900 order-1">
		<div class="flex justify-between items-center">
			<div class="flex justify-start items-center">
				<button
					id="dropdownUserCarButton"
					data-dropdown-toggle="dropdownUserCar"
					class="flex justify-between items-center p-2 rounded-lg dark:bg-gray-900 dark:hover:bg-gray-800 hover:bg-gray-50 dark:hover-bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700"
					type="button"
				>
					...
					<svg
						class="w-3 h-3 text-gray-500 dark:text-gray-400 ml-2"
						aria-hidden="true"
				...
				>
					{#each data.allCars as car}
						<a
							href={`/fuel/usages?currentUserId=${data.currentUser?.id}&currentCarId=${car.id}&pageIndex=1&pageSize=8`}
			...
			<div class="flex justify-between items-center lg:order-2">
				...
				>
					...
					<img
						...
						src={data.currentUser?.profileImageUrl}
						alt="user profile"
					/>
				</a>
			</div>
...
<nav>
	...
	>
		<div class="grid h-full max-w-lg grid-cols-3 mx-auto font-medium">
			...
			>
				<span
					class="text-md text-gray-500 dark:text-gray-400 group-hover:text-blue-600 dark:group-hover:text-blue-500"
					>เติมน้ำมัน</span
				>
			</a>
			<a
