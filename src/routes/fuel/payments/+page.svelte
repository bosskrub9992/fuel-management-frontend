<script lang="ts">
	...
	let selectedFuelUsageUserIds = $state<number[]>([]);
	let selectedFuelUsageTotalMoney = $state(0);
	let allFuelUsageUserIds = $state<number[]>([]);
	let totalMoneyIfSelectedAllFuelUsage = 0;
	let allSelectedFuelUsage = $derived(
		selectedFuelUsageUserIds.length == allFuelUsageUserIds.length
	);

	let selectedFuelRefillIds = $state<number[]>([]);
	let selectedFuelRefillTotalMoney = $state(0);
	let allFuelRefillIds = $state<number[]>([]);
	let totalMoneyIfSelectedAllFuelRefill = 0;
	let allSelectedFuelRefill = $derived(selectedFuelRefillIds.length == allFuelRefillIds.length);

	let isPaying = $state(false);
	let totalMoney = $derived(selectedFuelUsageTotalMoney - selectedFuelRefillTotalMoney);

	let userUnpaidCarActivity = $state<getUserUnpaidCarActivityResponse>();
	let isLoading = $state(true);
	let errorOccurred: unknown = $state();

	onMount(async () => {
		const { initFlowbite } = await import('flowbite');
		initFlowbite();

		let [err, res] = await getUserUnpaidCarActivity(data.currentUserId, data.currentCarId);
		if (err || res == undefined) {
			errorOccurred = err;
		}
		if (res) {
			userUnpaidCarActivity = res.userUnpaidCarActivity;

			selectedFuelUsageUserIds = res.selectedUnpaidFuelUsageUserIds;
			allFuelUsageUserIds = res.selectedUnpaidFuelUsageUserIds;
			selectedFuelUsageTotalMoney = res.totalMoneySelectedUnpaidFuelUsage;
			totalMoneyIfSelectedAllFuelUsage = res.totalMoneySelectedUnpaidFuelUsage;

			selectedFuelRefillIds = res.selectedUnpaidFuelRefillIds;
			allFuelRefillIds = res.selectedUnpaidFuelRefillIds;
			selectedFuelRefillTotalMoney = res.totalMoneySelectedUnpaidFuelRefill;
			totalMoneyIfSelectedAllFuelRefill = res.totalMoneySelectedUnpaidFuelRefill;
		}
	...
	let showToast = $derived($page.url.searchParams.get('showToast'));
...
{:else if userUnpaidCarActivity}
	{#if userUnpaidCarActivity.fuelUsages.length == 0 && userUnpaidCarActivity.fuelRefills.length == 0}
									...
									<tbody>
										{#each userUnpaidCarActivity.fuelRefills as fuelRefill}
											<tr class="border-b dark:border-gray-700">
												...
												>
												<td
													id="td-fuel-refill-{fuelRefill.fuelRefillId}-TotalMoney"
													class="px-2 py-3">{'฿' + fuelRefill.totalMoney}</td
												>
											</tr>
										{/each}
									</tbody>
