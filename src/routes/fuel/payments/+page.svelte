<script lang="ts">
    ...
    import type { getUserUnpaidFuelUsagesResponse } from '$lib/users';
	import { 
        PUBLIC_BASE_URL, 
        PUBLIC_USERS_ENDPOINT,
        PUBLIC_BANK_ACCOUNT_NO, 
        PUBLIC_BANK 
    } from '$env/static/public';

	export let data: PageData;

	let promiseUserUnpaidFuelUsage: Promise<getUserUnpaidFuelUsagesResponse> | null = null;
	...
	let urlGetUserUnpaidFuelUsages = `${PUBLIC_BASE_URL}${PUBLIC_USERS_ENDPOINT}/${data.currentUserId}/fuel-usages?isPaid=false`;
...
{#if browser}
    ...
    {:then userUnpaidFuelUsages}
        {#if userUnpaidFuelUsages }
            {#if userUnpaidFuelUsages.userFuelUsages.length == 0 }
                <div class="flex flex-col items-center justify-center">
                    <p class="text-lg">ไม่มีรายการค้างจ่ายครับ</p>
                    <p class="text-lg">🎉🎉🎉</p>
                </div>  
            {:else}
                <p class="text-lg font-bold px-4 py-2">รายการรอคุณจ่าย</p>
                {#each userUnpaidFuelUsages.userFuelUsages as userFuelUsage }
                    <!-- table -->
                                        ...
                                        <tbody>
                                            {#each userFuelUsage.fuelUsages as fuelUsage }
                                                <tr class="border-b dark:border-gray-700">
                                                    ...
                                                    <td id="td-{fuelUsage.fuelUsageUserId}-FuelUsetime" class="px-2 py-3"> {fuelUsage.fuelUseTime}</td>
                                                    <td id="td-{fuelUsage.fuelUsageUserId}-FuelUsers" class="px-2 py-3 align-middle">{fuelUsage.fuelUsers}</td>
                                                    <td id="td-{fuelUsage.fuelUsageUserId}-Description" class="px-2 py-3">{fuelUsage.description}</td>
                                                    <td id="td-{fuelUsage.fuelUsageUserId}-PayEach" class="px-2 py-3">{"฿" + fuelUsage.payEach}</td>
                                                </tr>
                                            {/each}
                                        </tbody>
                                    </table>
