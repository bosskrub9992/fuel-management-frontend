<script lang="ts">
    ...
    import type { getUserFuelUsagesResponse, patchUserFuelUsagesPaymentStatusRequest} from "$lib/api/users";
        ...
        try {
            ...
            for (let carIdx = 0; carIdx < userUnpaidFuelUsages.userFuelUsages.length; carIdx++) {
                ...
                for (const fuelUsage of userUnpaidFuelUsages.userFuelUsages[carIdx].fuelUsages) {
                    $cars[carIdx] = {
                        index: carIdx,
                        selectedFuelUsageUserIds: [...$cars[carIdx].selectedFuelUsageUserIds, fuelUsage.fuelUsageUserId],
    ...
    cars.subscribe(($cars) => {
        for (const c of $cars) {
            $cars[c.index].allSelected = c.allFuelUsageUserIds.length === c.selectedFuelUsageUserIds.length
        }
    });
    ...
    function pay(carIdx: number) {
        ...
        for (const selectedFuelUsageUserId of $cars[carIdx].selectedFuelUsageUserIds) {
            req.userFuelUsages = [...req.userFuelUsages, {
                                        ...
                                        <td class="w-4 p-4">
                                            <div class="flex items-center">
                                                <input
                                                    ...
                                                    on:change={(e)=>{calculateTotalMoney(e, i, parseFloat(fuelUsage.payEach))}}
                                                    class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                                                >
                                                <label for="checkbox-{fuelUsage.fuelUsageUserId}" class="sr-only">checkbox</label>
                                            </div>
                                        </td>
