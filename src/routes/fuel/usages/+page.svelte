...
{:else if  data.showToast === "failed" || data.showToast === "updateFailed" || data.showToast === "deleteFailed"}
	<div id="toast-danger" class="flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 mx-auto" role="alert">
				...
				<table class="w-full text-sm text-center text-gray-500 dark:text-gray-400">
					...
					<tbody>
						{#each fuelUsageData as fuelUsageDatum }
							<tr on:click={()=>{goto(`/fuel/usages/${fuelUsageDatum.id}?currentUserId=${data.currentUser?.id}&currentCarId=${data.currentCar?.id}`)}} class="border-b dark:border-gray-700">
